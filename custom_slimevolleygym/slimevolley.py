...
def rect(canvas, x, y, width, height, color):
  ...
  if PIXEL_MODE:
    ...
    rect = pygame.Rect(round(x), round(WINDOW_HEIGHT-y), round(width), round(height))
...
def half_circle(canvas, x, y, r, color):
  ...
  if PIXEL_MODE:
    ...
    pygame.draw.circle(canvas, color, (round(x), WINDOW_HEIGHT-round(y)), round(r), draw_top_left=True, draw_top_right=True)
    return canvas
...
def circle(canvas, x, y, r, color):
  ...
  if PIXEL_MODE:
    ...
    pygame.draw.circle(canvas, color, (round(x), WINDOW_HEIGHT-round(y)), round(r))
    return canvas
...
class SlimeVolleyEnv(gym.Env):
  ...
  def step(self, action, otherAction=None):
    ...
    if self.survival_bonus:
      return obs, float(reward)+0.01, terminated, truncated, info
...
register(
    id='SlimeVolley-v0',
    entry_point='custom_slimevolleygym.slimevolley:SlimeVolleyEnv'
)

register(
    id='SlimeVolleyPixel-v0',
    entry_point='custom_slimevolleygym.slimevolley:SlimeVolleyPixelEnv'
)

register(
    id='SlimeVolleyNoFrameskip-v0',
    entry_point='custom_slimevolleygym.slimevolley:SlimeVolleyAtariEnv'
)

register(
    id='SlimeVolleySurvivalNoFrameskip-v0',
    entry_point='custom_slimevolleygym.slimevolley:SlimeVolleySurvivalAtariEnv'
)
...
if __name__=="__main__":
  ...
  Humans can override controls using pygame key events:
  ...
  # Pygame key handling
  def handle_pygame_keys():
    global manualMode, manualAction, otherManualMode, otherManualAction
    keys = pygame.key.get_pressed()
    
    # Right agent controls (arrow keys)
    manualAction[0] = 1 if keys[pygame.K_LEFT] else 0
    manualAction[1] = 1 if keys[pygame.K_RIGHT] else 0
    manualAction[2] = 1 if keys[pygame.K_UP] else 0
    if any(manualAction):
      manualMode = True
    
    # Left agent controls (WASD)
    otherManualAction[0] = 1 if keys[pygame.K_d] else 0
    otherManualAction[1] = 1 if keys[pygame.K_a] else 0
    otherManualAction[2] = 1 if keys[pygame.K_w] else 0
    if any(otherManualAction):
  ...
  while not done:
    # Handle pygame events and key input
    if RENDER_MODE:
      handle_pygame_keys()
    ...
    if otherManualMode:
      ...
      obs, reward, done, truncated, info = env.step(action, otherAction)
    else:
